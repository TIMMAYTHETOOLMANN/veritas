# zk/witness.py — Layer 2: Adversarial Witness Generator (ENHANCED)
# Generates malformed witness vectors targeting all 5 exploit classes:
...
# ---------------------------------------------------------------------------
# Class 1: ZK-FIELD-OVERFLOW
# ---------------------------------------------------------------------------
def gen_field_overflow(n_inputs, rng, unconstrained=None):
...
# ---------------------------------------------------------------------------
# Class 3: ZK-NULLIFIER-COLLISION
# ---------------------------------------------------------------------------
def gen_nullifier_collision(rng, n_pairs=4):
    """Pairs of near-identical secrets to hunt nullifier collisions."""
    out = []
...
# ---------------------------------------------------------------------------
# Class 4: ZK-VERIFIER-CONFIG-MISMATCH
# ---------------------------------------------------------------------------
def gen_config_mismatch(vk_a, vk_b, rng, n_inputs=4):
    """Proof replay across circuits — the (A,B,C) stays, the binding changes."""
    return {
        "class": "ZK-VERIFIER-CONFIG-MISMATCH",
        "vk_a": vk_a.get("vk_hash") if isinstance(vk_a, dict) else vk_a,
        "vk_b": vk_b.get("vk_hash") if isinstance(vk_b, dict) else vk_b,
        "witness": [rng.randrange(R) for _ in range(n_inputs)],
    }


# ---------------------------------------------------------------------------
# Class 5: ZK-PROOF-MALLEABILITY
# ---------------------------------------------------------------------------
def gen_malleability(proof_pi=None, rng=None):
    """Mutate a valid proof (A,B,C): negate A, swap B coordinates, etc."""
    if proof_pi is None:
        rng = rng or rng_default()
        proof_pi = [rng.randrange(R) for _ in range(8)]
    mutations = {
...
def generate_corpus(spec, seed=0x5EED, n=256):
    ...
    # Class 1: Field overflow
    for _ in range(per_class):
        corpus.append(gen_field_overflow(n_in, rng))

    # Class 2: Under-constrained
    for _ in range(per_class):
        corpus.append(gen_garbage(n_in, rng, unc))

    # Class 3: Nullifier collision (multiple pairs)
    for pair in gen_nullifier_collision(rng, n_pairs=max(1, per_class // 4)):
        corpus.append(pair)

    # Class 4: Config mismatch
    if spec.get("vk_hash"):
        for _ in range(max(1, per_class // 4)):
            corpus.append(gen_config_mismatch(spec, spec, rng))

    # Class 5: Proof malleability
    if spec.get("vk_hash"):
        corpus.append(gen_malleability([rng.randrange(R) for _ in range(8)], rng))

    return corpus


def generate_targeted_corpus(vclass, spec, n=64, seed=0x5EED):
    """Generate a corpus targeting a SPECIFIC exploit class."""
    rng = random.Random(seed)
    n_in = spec.get("n_inputs", 8)
    unc = spec.get("unconstrained") or []

    if vclass == "ZK-FIELD-OVERFLOW":
        return [gen_field_overflow(n_in, rng) for _ in range(n)]
    elif vclass == "ZK-UNDER-CONSTRAINED":
        return [gen_garbage(n_in, rng, unc) for _ in range(n)]
    elif vclass == "ZK-NULLIFIER-COLLISION":
        return gen_nullifier_collision(rng, n_pairs=n)
    elif vclass == "ZK-VERIFIER-CONFIG-MISMATCH":
        return [gen_config_mismatch(spec, spec, rng) for _ in range(n)]
    elif vclass == "ZK-PROOF-MALLEABILITY":
        return [gen_malleability(rng=rng) for _ in range(n)]
    else:
        return generate_corpus(spec, seed=seed, n=n)
