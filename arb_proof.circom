 ...
 *
 * NOTE ON PUBLIC SIGNALS (circom 2.2.3 semantics):
 *   `signal input` in `main` compiles as PRIVATE inputs. Only `signal output`
 *   becomes public. To make the verifier-visible context (state_root, eth_usd,
 *   gas_usd, safety_margin) public, they MUST be re-declared as `signal output`.
 *
 *   snarkjs public-signal order = OUTPUTS in declaration order;
 *   then (if any) INPUTS. With only outputs populated, order is:
 *     [0] nullifier
 *     [1] profit_usd
 *     [2] net_profit_usd
 *     [3] state_root
...
template ArbProofVerifier() {
    // === CONTEXT INPUTS (consumed in-circuit; re-exposed as outputs below) ===
    ...
    // profit_usd = profit_weth * eth_usd / 1e18
    //   profit_weth is 1e18-scaled WETH, eth_usd is 1e6-scaled USD/ETH
    //   => result is 1e6-scaled USD.
    signal profit_usd;
    ...
    //   [3] state_root, [4] eth_usd, [5] gas_usd, [6] safety_margin
    // Context values re-exposed as outputs so the on-chain verifier can
    // check state freshness and safety margin.
    ...
    signal output state_root_out;
    signal output eth_usd_out;
    signal output gas_usd_out;
    signal output safety_margin_out;
    state_root_out <== state_root;
    eth_usd_out <== eth_usd;
    gas_usd_out <== gas_usd;
    safety_margin_out <== safety_margin;
}
